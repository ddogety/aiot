...
try:
    while 1: #프로그램이 강제로 꺼질때까지 무한반복
        leds.value = (0,0,1,1,0) #GPIO 4,20번을 켜진상태로 변경 나머지는꺼짐
        sleep(3.0) #sleep()은 대기 시간을 설정 4,20번이 켜진상태로 3초대기
        leds.value = (0,1,0,1,0) #GPIO 3,20번을 켜진상태로 변경 나머지는꺼짐
        sleep(1.0) #3,20번이 켜진상태로 1초대기
        leds.value = (1,0,0,0,1) #GPIO 2,21번을 켜진상태로 변경 나머지는꺼짐
        sleep(3.0) #3,21번이 켜진상태로 3초대기
